...
export default function About() {
  ...
  // ===== DOOM SCROLL TIMELINE — GSAP ScrollTrigger =====
  useEffect(() => {
    if (!scrollerRef.current || !pinnedRef.current) return

    const total = TIMELINE_ITEMS.length

    // Pin the viewport div for (total * 100vh) of scrolling
  ...
  const interests = [
    ...
    { icon: '🥊', title: 'Boxe', desc: "Un peu de sport n'a jamais tué qui que ce soit.", highlight: true },
  ]
  ...
  while (world.hasSecrets()) {
    ...
    if (clue) decode(clue);
      ...
      <section className="timeline-section">
        ...
        {/* scrollerRef defines the total scroll height */}
        <div ref={scrollerRef} className="tl-scroller">

          {/* pinnedRef is the element that gets pinned (sticky) */}
          <div ref={pinnedRef} className="tl-pinned">
            ...
            {/* Card stage — perspective on parent, preserve-3d */}
                ...
                >
                  <div className="tl-card-inner">
                    ...
                    <div className="tl-card-header">
                      <span className="tl-card-num">
                        {String(i + 1).padStart(2, '0')} / {String(TIMELINE_ITEMS.length).padStart(2, '0')}
                      </span>
                    ...
                    <div className="tl-card-tags">
                      {item.tags.map(tag => (
                        <span key={tag} className="tl-card-tag">{tag}</span>
                      ))}
                    </div>
                  </div>
                </div>
            ...
            <div className="tl-progress-bar">
              <div className="tl-progress-fill"
                style={{ width: `${((activeCard + 1) / TIMELINE_ITEMS.length) * 100}%` }} />
            </div>
